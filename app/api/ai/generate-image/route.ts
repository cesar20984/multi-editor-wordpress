    ...
    try {
        ...
        if (finalModel.includes("imagen")) {
            ...
            if (finalModel === "dall-e-2") {
                ...
                else if (imageSize === "256px") dallESize = "256x256";
            } else {
                // DALL-E 3 & gpt-image support non-square standard sizes
                if (imageAspectRatio === "16:9" || imageAspectRatio === "4:3") dallESize = "1792x1024";
                else if (imageAspectRatio === "9:16" || imageAspectRatio === "3:4") dallESize = "1024x1792";
                else dallESize = "1024x1024"; // 1:1 fallback
            }
            ...
            if (!finalModel.startsWith("gpt-image")) {
                payload.response_format = "url";
            }

            const imageResponse = await openai.images.generate(payload);
            ...
            if (imageResponse.data[0].b64_json) {
                imgBuffer = Buffer.from(imageResponse.data[0].b64_json, "base64");
            } else {
                imageUrl = imageResponse.data[0].url || "";
                if (!imageUrl) throw new Error("No URL or b64_json returned by OpenAI");
